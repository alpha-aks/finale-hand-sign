...
def _dir_has_any_subdirs(dir_path: Path) -> bool:
    if not dir_path.exists() or not dir_path.is_dir():
        return False
    return any(child.is_dir() for child in dir_path.iterdir())


_primary_root = _THIS_DIR
_fallback_root = _THIS_DIR / "hand-sign"

_primary_training_dir = _primary_root / "training_data"
_fallback_training_dir = _fallback_root / "training_data"

_data_root = _primary_root
if (not _dir_has_any_subdirs(_primary_training_dir)) and _dir_has_any_subdirs(_fallback_training_dir):
    _data_root = _fallback_root

BASE_DATA_DIR = str(_data_root / "training_data")
MODELS_DIR = str(_data_root / "trained_models")
CONFIG_FILE = str(_data_root / "gesture_config.json")
...
def create_gesture_recognizer():
    # Prefer landmark KNN (gesture-name-driven) to avoid color-histogram confusion.
    mp_model_ok = False
...
class MediaPipeHandsDetector:
    ...
    def detect_hands(self, image):
        try:
            ...
            # Heuristics to reduce false positives on background objects.
...
def main():
    ...
    mode = st.sidebar.radio(
        "Select Mode",
        ["📚 Train New Gesture", "🔍 Test & Recognize", "🔤 Alphabet (Train + Type)", "📊 View Training Data"],
        index=0
    )
...
def alphabet_mode():
    ...
    with tab_train:
        ...
        if not st.session_state.get('alphabet_is_capturing'):
            # Show a quick per-letter summary
        ...
        max_seconds = 12.0  # keep the loop bounded; Streamlit reruns after this
        t0 = time.time()
        ...
        while st.session_state.get('alphabet_is_capturing') and (time.time() - t0) < max_seconds:
            ...
            if has_hand and (now - last_capture_time) >= float(capture_delay):
                ...
                # Advance letter if reached target
                current_count = get_gesture_image_count(current_letter)
                    ...
                    try:
                        if not new_gesture:
                            ...
                            if src_gesture and src_gesture != new_gesture:
                                # Keep config tidy if the old folder is gone/empty
